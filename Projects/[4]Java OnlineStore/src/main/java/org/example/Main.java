...
import static org.example.UserManager.getUser;
...
public class Main {
    ...
    private static void addProductStore(Scanner input) throws SQLException {
        ...
        System.out.println("2. Alcoholic Beverage");
        int choice = input.nextInt();
        if (choice == 1) {
    ...
    private static void addProductToCart(Scanner input) throws SQLException {
        ...
        if(!resultSet.next()){
            InexistentProductException exception = new InexistentProductException(productName);
            throw exception;

            }
        }catch (InexistentProductException exception){
            System.out.println("The prouct " + productName + " you want to add to cart doesn't exist.");
        }
